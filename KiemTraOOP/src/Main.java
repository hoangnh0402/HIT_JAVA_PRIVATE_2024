...
public class Main {
    public static void main(String[] args) {
        ...
        accountService.createNewAccount(new Account("A001", "Nguyen Van Admin", 35, adminAddress, "1989-01-01", "admin", "1234", Role.ADMIN, Status.ACTIVE));

        Address userAddress = new Address("456 User St", "User City");
        accountService.createNewAccount(new Account("U001", "Nguyen Van User", 30, userAddress, "1993-05-15", "user", "abcd", Role.USER, Status.ACTIVE));
        ...
        while (true) {
            if (authService.getLoggedInAccount() == null) {
                ...
                switch (choice) {
                    ...
                    case 3:
                        if (loggedInAccount.getRole() == Role.ADMIN) {
                            System.out.println("Nhập thông tin tài khoản mới (username, password, full name, age, street, city, birthday, status): ");
                            ...
                            Account newAccount = new Account("", fullName, age, new Address(street, city), birthday, username, password, Role.USER, status);
